...
class Item(Base):
    ...
    category: Mapped[str] = mapped_column(String(32), default='misc')  # tool|consumable|booster|crate|collectible
    stackable: Mapped[bool] = mapped_column(Boolean, default=True)
    consumable: Mapped[bool] = mapped_column(Boolean, default=False)
    limited: Mapped[bool] = mapped_column(Boolean, default=False)   # not sold in shop (crate/event only)
    rarity: Mapped[str] = mapped_column(String(16), default='common')  # common|uncommon|rare|epic|legendary
    emoji: Mapped[str] = mapped_column(String(8), default='📦')
    max_stack: Mapped[int] = mapped_column(Integer, default=99)
    expires_in: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # seconds after acquire (0/None = never)
    effects: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON string of usage effects
