...
from models import Transaction, UserAchievement, Wallet
...
ACHIEVEMENTS: Dict[str, dict] = {
    'first_steps':   {"name": "First Steps",   "desc": "Work for the first time",                "emoji": "💼"},
    'criminal':      {"name": "Criminal",      "desc": "Commit your first crime",                "emoji": "🕶️"},
    'master_thief':  {"name": "Master Thief",  "desc": "Successfully rob 10 times",              "emoji": "🦹"},
    'gambler':       {"name": "Gambler",       "desc": "Place your first gamble",                "emoji": "🎲"},
    'high_roller':   {"name": "High Roller",   "desc": "Gamble 10,000 coins in one bet",         "emoji": "💸"},
    'shopaholic':    {"name": "Shopaholic",    "desc": "Make your first shop purchase",          "emoji": "🛒"},
    'collector':     {"name": "Collector",     "desc": "Own 5 different items",                  "emoji": "🧺"},
    'explorer':      {"name": "Explorer",      "desc": "Search 5 times",                         "emoji": "🔍"},
    'streak_3':      {"name": "On Fire",       "desc": "Reach a 3-day daily streak",             "emoji": "🔥"},
    'streak_7':      {"name": "Unstoppable",   "desc": "Reach a 7-day daily streak",             "emoji": "🌋"},
    'rich':          {"name": "Rich",          "desc": "Reach 100,000 net worth",                "emoji": "💰"},
    'millionaire':   {"name": "Millionaire",   "desc": "Reach 1,000,000 net worth",              "emoji": "🤑"},
    'prestige_1':    {"name": "Ascended",      "desc": "Prestige for the first time",            "emoji": "👑"},
}
...
class AchievementService:
    ...
    async def _stats(session: AsyncSession, user_id: int) -> dict:
        ...
        counts: dict = {}
        for tx_type in ('work', 'crime', 'rob', 'gamble', 'search', 'buy', 'crate', 'item'):
...
class ProgressionService:
    ...
    async def apply_daily(
        ...
        async with lock_manager.for_user(user_id):
            ...
            if wallet.last_daily_at and (now - wallet.last_daily_at) < timedelta(hours=24):
                ...
                return False, f"⏰ Already claimed today. Try again in **{hours_left:.1f} hours**.", 0, wallet.daily_streak or 0
    ...
    @staticmethod
    async def apply_monthly(session: AsyncSession, user_id: int, base_reward: int) -> Tuple[bool, str]:
        ...
        async with lock_manager.for_user(user_id):
            ...
            if wallet.last_monthly_at and (now - wallet.last_monthly_at) < timedelta(days=30):
                ...
                return False, f"❌ Monthly reward already claimed. Try again in **{remaining} day(s)**."
    ...
    async def prestige(session: AsyncSession, user_id: int) -> Tuple[bool, str]:
        ...
        async with lock_manager.for_user(user_id):
            ...
            session.add(
                Transaction(
                    user_id=user_id, type='prestige', amount=-networth,
            ...
            wallet.bank = 0
            await session.commit()
            return True, f"✨ You reached **Prestige {wallet.prestige}**! All rewards +{int(ProgressionService._prestige_multiplier(wallet) * 100 - 100)}%"
