...
class AntiFraud:
    ...
    def __init__(self):
        self.bet_history: Dict[int, List[tuple]] = defaultdict(
            list
    ...
    def check_large_bets(
        self, user_id: int, amount: int, threshold: int = 10000
    ) -> tuple[bool, str]:
        """Check for large bet amounts."""
        if amount > threshold:
            return True, f"Large bet of {amount} 💎️ flagged for review."
    ...
    def check_transfer_patterns(self, user_id: int) -> tuple[bool, str]:
        ...
        now = time.time()
        recent_transfers = [
            t for t, _ in self.transfer_history[user_id] if now - t < 3600
        ]  # Last hour
